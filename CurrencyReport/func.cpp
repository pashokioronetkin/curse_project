...
void get_current_date1(char* date_str) {
    time_t t = time(NULL); // Получение текущего времени
    struct tm tm = *localtime(&t); // Преобразование времени в локальное время
...
bool check_exchange_id_exists(const char* fname, int exchange_id) {
...
void do_query(const Exchange exchanges[], int limit, const char* fname) {
    if (limit == 0) {
...
void do_query1(const Exchange exchanges[], int limit, const char* fname) {
    if (limit == 0) {
