...
int main() {
    ...
    Exchange exchanges[ExchangeSystem::AR_EXCHANGE_LEN]; // Массив для хранения обменов
    Manager managers[ExchangeSystem::AR_MAN_LEN];       // Массив для хранения менеджеров

    // Загрузка данных о менеджерах из файла
    man_count = ExchangeSystem::load_managers("managers.txt", managers, ExchangeSystem::AR_MAN_LEN);
    ...
    while (item != 0) {
        ...
        switch (item) {
            ...
            case 2:
                // Вход для менеджера
                ExchangeSystem::manager_login(managers, man_count, exchanges, &exchange_count);
                break;
            case 3:
                // Вход для администратора
                ExchangeSystem::admin_login(managers, &man_count);
                break;
            default:
